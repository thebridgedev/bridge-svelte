...
async function preSetup() {
  ...
  // Configure allowedOrigins so SDK auth (SdkOriginGuard) accepts localhost requests
  console.log(`[pre-setup] Configuring allowedOrigins for SDK auth...`);
  const configRes = await fetch(`${testDataApiUrl}/account/test/playwright/configure-app`, {
    method: 'POST',
    headers: {
      'Content-Type': 'application/json',
      'x-playwright-api-key': apiKey,
  ...
  if (!configRes.ok) {
    ...
    console.warn(`[pre-setup] Warning: Failed to configure allowedOrigins: ${configRes.status} ${error}`);
  } else {
    console.log(`[pre-setup] allowedOrigins configured`);
  }

  // Write a Playwright storage state file with the app ID in localStorage.
  // Covers both the Docker demo (port 3008) and the local Vite dev server (port 3001)
  // so tests work regardless of which is running.
  const authDir = path.resolve(rootDir, 'e2e/playwright/.auth');
  if (!fs.existsSync(authDir)) fs.mkdirSync(authDir, { recursive: true });

  const baseState = {
    cookies: [],
    origins: [
      { origin: 'http://localhost:3001', localStorage: [{ name: 'bridge:appId', value: appId }] },
      { origin: 'http://localhost:3008', localStorage: [{ name: 'bridge:appId', value: appId }] },
    ],
  };
  const baseStateFile = path.resolve(authDir, 'base-state.json');
  fs.writeFileSync(baseStateFile, JSON.stringify(baseState, null, 2));
  console.log(`[pre-setup] Wrote bridge:appId=${appId} to ${baseStateFile}`);
