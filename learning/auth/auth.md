...
**Under the hood:** both modes hit the same backend endpoint `GET /auth/auth/federation/:appId?provider=<type>`, which builds a full, validated OAuth context through the same pipeline as `/auth/authorize` — `redirect_uri` is validated against the app's `allowedRedirectUris` (falling back to its `defaultCallbackUri`), `scope`/`response_type` are defaulted, and a CSRF-safe `state` is minted server-side when the caller didn't supply one. Redirect mode sends `redirect_uri=<your callbackUrl>` (overridable per-call via `SsoOptions.redirectUri`) so the provider round-trip lands back on your app's callback with both `code` and `state` populated. Popup mode additionally sends `mode=popup&targetOrigin=<origin>` query params, which the backend uses to route the final callback into a `postMessage` instead of a normal redirect.
