...
## Billing state is already live — no init call
...
the workspace subscription, auto-mounts the billing notice/gate, and honors your
configured billing routes. There is **no separate billing init call**.

State lands on the unified `bridge` surface and updates over the live channel:
...
**1. Letting users select a plan after first sign-up** — wrap your root layout in
...
> [usage limits](/billing/limits/usage-limits/), free trials, the billing
> portal, and failed-payment handling — each building on the live `bridge`
> surface you now have wired up.
