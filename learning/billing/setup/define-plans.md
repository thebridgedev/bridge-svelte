...
the next step is to define the plans your workspaces can subscribe to.
...
- **Quotas** (optional) — usage caps for a metric, e.g. 10,000 AI calls a month.
  See [Set usage limits](/billing/limits/usage-limits/) for how these surface in
  your app.
...
You can define plans with the CLI, the Bridge dashboard, or the REST API — pick
whichever fits your workflow. All three write to the same place.
...
- `plan quota set <key>` takes `--metric`, `--limit`, and `--policy`
  ...
  price). Inspect a plan and its quotas with `bridge plan get <key>`.
...
Prefer a UI? Create and edit plans, prices, and quotas in the
[Bridge dashboard](https://app.thebridge.dev/subscriptions?tab=plans) — the same
