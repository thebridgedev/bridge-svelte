...
Practically, that means an admin can open Control Center and write a rule like "on for `role = ADMIN`" or "on for `tenant.plan = ENTERPRISE`" for any flag, the moment auth or billing is connected — nothing to send from your code, nothing to redeploy.
...
Your own (dev-supplied) attributes always win on key collision with any of these, and the admin UI surfaces the collision on the flag detail page so it's never silently confusing.
