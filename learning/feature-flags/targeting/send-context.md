...
`<FeatureFlag>` takes the same context as a `context` prop instead of a third argument, and reactivity works slightly differently between the two; see [Show or hide UI](/feature-flags/using/show-hide-ui/#sending-context) for the component form and the difference.
